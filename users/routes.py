...
def save_cred():
    try:
        ...
        data = {"username": username, "password": password,"email":email, "bookmarks": []}
...
def upload():
    if request.method == 'POST':
        ...
        if video_file and allowed_file(video_file.filename):
           ...
           #we are converting video file (binary data ) to base64 representation.it is bytes like object which contains base64 representation. then we are decoding it using utf 8 which converts bytes like object to string  
...
def rate_video():
    try:
        ...
        video_data = mongo.db.videoss.find_one({"_id": ObjectId(video_id)})
        ratings = video_data.get('ratings', [])
        if ratings:
            avg_rating = sum(ratings) / len(ratings)
        else:
            avg_rating = 0.0
        return jsonify({
            'success': "Video rated successfully!",
...
def display():
    ...
    if search_query:
        # Use a regular expression to search for tags containing the search query
        regex = re.compile(re.escape(search_query), re.IGNORECASE)
        videos = mongo.db.videoss.find({ '$or': [ {'tags': regex}, {'title': regex},{'createdBy':regex}]})
...
@routes_bp.route('/myfavorites', methods=['GET'])
def favourite_video():
    try:
        username = request.args["username"]
        print("username", username)
        user_data = is_valid_username(username, mongo)
        user_data["_id"] = str(user_data["_id"])
        return jsonify({'success': "Video rated successfully!", 'user_data': user_data}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 400
...
def bookmark_video():
    try:
        ...
        # Update the video's rating in the database
        mongo.db.usersa.update_one(
            {"username": session["user"]},
            {"$push": {"bookmarks": ObjectId(video_id)}}
        )
...
@routes_bp.route('/myfavoritesvideos', methods=['GET'])
def MyFavoriteVideos():
    # Fetch all videos created by the logged-in user from the database
    user_data = is_valid_username(session["user"], mongo)
    user_data["_id"] = str(user_data["_id"])
    videos = mongo.db.videoss.find({'_id': {'$in': user_data["bookmarks"]}})
    allVideos = []
    for video in videos:
        video_id = str(video["_id"])
